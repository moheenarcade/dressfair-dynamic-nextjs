...
const ProductDetailMain = ({ productDetail }) => {
    ...
    const handleAddToCart = () => {
        ...
        if (isSelectedSizeOutOfStock()) {
            showToast("This product is out of stock for the selected size");
            return;
        }

        // Check if requested quantity is more than available
        if (quantity > Number(selectedSizeObj.available_quantity)) {
            showToast(`Only ${selectedSizeObj.available_quantity} items available in this size`);
            return;
        }
    ...
    return (
        <>
            ...
            {/* Toast Notification */}
            {toast.show && (
                <div className={`fixed top-4 right-4 z-[99999] px-6 py-3 rounded-lg shadow-lg font-semibold text-white ${
